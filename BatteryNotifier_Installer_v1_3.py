...
class BatteryNotifier:
    ...
    def check_battery(self, icon):
        alarm_cooldown = 0  # counts up in 1-second ticks
        ...
        while self.running:
            ...
            # --- Real-time: update tooltip, menu label, and color icon every second ---
            self.current_percent = percent
            self.current_plugged = plugged
            icon.title = f"Battery Notifier - {percent}%"

            if percent <= self.low_value:
                icon.icon = Image.open(resource_path("icon_red.ico"))
            elif percent >= self.full_value:
                icon.icon = Image.open(resource_path("icon_green.ico"))
            else:
                icon.icon = Image.open(resource_path("icon_white.ico"))

            # --- Alarm logic: only evaluated every 10 seconds ---
            alarm_cooldown += 1
            if alarm_cooldown >= 10:
                alarm_cooldown = 0
                try:
                    if percent <= self.low_value and not plugged:
                        ...
                        # Reset triggers when battery is in normal range
                        self.low_alert_triggered = False
                        self.full_alert_triggered = False
...
def create_tray_icon(notifier):
    image = Image.open(resource_path("icon_white.ico"))
    ...
    menu = Menu(
        MenuItem(
            lambda item: f"🔋 {notifier.current_percent}% {'(Charging)' if notifier.current_plugged else '(Unplugged)'}",
            None,
            enabled=False
        ),
